...
const HTML = {
	BTN_INNER: '<div id="accManagerBtn" class="button buttonB bigShadowT" onmouseenter="playTick()" style="display:block;width:300px;text-align:center;padding:15px;font-size:23px;pointer-events:all;padding-bottom:22px;margin-left:-5px;margin-top:5px">iAccounts</div>',
...
class AccountManager {
	...
	#watcher = () => {
		...
		document.querySelectorAll(".altdeletebtn").forEach(i => i.addEventListener("click", (e) => {
			...
			confirm(`[WARN] Please confirm that you wanna remove "${tar}" from the iAccounts List?`) && this.#deleteAccount(tar);
		}));
	};
